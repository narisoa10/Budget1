...
class MiscellaneousPageState extends State<MiscellaneousPage> {
  ...
  Future<void> _loadSettings() async {
    ...
    List<String>? savedNewCategories = settingsBox.get('misc_new_categories')?.cast<String>();
  ...
  Future<void> _saveNewCategories() async {
    await settingsBox.put('misc_new_categories', _newCategories);
  }
  ...
  void _updateCategoryExpense(String category, double newSum) {
    if (_categoryExpenses[category] != newSum) {
      ...
      settingsModel.saveCategoryExpense(
        mainCategory: "misc_expenses",
        subCategory: category,
        amount: newSum,
        date: currentDate,
      );

      settingsModel.updateCategoryExpense("misc_expenses", _totalExpense);
    }
  }

  @override
  Widget build(BuildContext context) {
    String pageTitle = Localization.getTranslation('misc_expenses');
        ...
        children: [
          ...
          SizedBox(
            ...
            child: TotalContainer(
              ...
              categoryName: 'misc_expenses', // Указание категории
            ),
          ),
        ],
      ),

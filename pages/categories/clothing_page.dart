...
class ClothingPage extends StatefulWidget {
  ...
  @override
  ClothingPageState createState() => ClothingPageState();
}

class ClothingPageState extends State<ClothingPage> {
  double _totalExpense = 0.0; // Переменная для общей суммы расходов
  final Map<String, double> _categoryExpenses = {
    "cloth_clothing": 0.0,
    "cloth_footwear": 0.0,
    "cloth_hair_salons": 0.0,
    "cloth_cosmetics_skincare": 0.0,
    "cloth_personal_hygiene": 0.0,
    "cloth_paid_apps_subscriptions": 0.0,
    "cloth_fitness_body_care": 0.0,
    "cloth_mobile_internet": 0.0,
  };
  ...
  final List<String> _categories = [
    ...
    'cloth_footwear',
    'cloth_hair_salons',
    'cloth_cosmetics_skincare',
    'cloth_personal_hygiene',
    'cloth_paid_apps_subscriptions',
    'cloth_fitness_body_care',
    'cloth_mobile_internet',
  ];
  ...
  Future<void> _loadSettings() async {
    ...
    settingsBox.get('clothing_new_categories')?.cast<String>();
  ...
  Future<void> _saveNewCategories() async {
    await settingsBox.put('clothing_new_categories', _newCategories);
  }
  ...
  Future<void> _addNewCategory(String categoryName) async {
    if (categoryName.isNotEmpty &&
      ...
      setState(() {
        ...
        _categoryExpenses[categoryName] = 0.0; // Добавляем новую категорию в карту
      });
  ...
  Future<void> _removeCategory(String categoryName) async {
    if (_newCategories.contains(categoryName)) {
      ...
      setState(() {
        ...
        _categoryExpenses.remove(categoryName); // Удаляем из карты расходов
  ...
  void _updateCategoryExpense(String category, double newSum) {
    if (_categoryExpenses[category] != newSum) { // Обновляем только при изменении
      ...
      settingsModel.updateCategoryExpense("clothing", _totalExpense);
    }
  }

  @override
  Widget build(BuildContext context) {
    String pageTitle = Localization.getTranslation('clothing'); // Заголовок страницы
    ...
    return ChangeNotifierProvider(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Table1(
              onTotalSumChanged: (newSum) => _updateCategoryExpense(
                  categoryKey, newSum),
              title: title,
            ),
          ],
